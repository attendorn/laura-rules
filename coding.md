...
**Ein Rahmen je Repo (Florian-Entscheid 12.09.2026, Memory `7c45e21f`).** In laura-pa und agent-basis ist **pytest der einzige Rahmen für neue Tests**; `bats` ist eingefroren: kein neuer bats-Test, portiert wird nur, was langsam ist oder ohnehin angefasst wird. Repos mit TypeScript-Kern behalten ihren eigenen einen Rahmen. Nie zwei Rahmen im selben Repo für dieselbe Schicht. Ein Shell-Hook wird darin ehrlich geprüft, indem der Test ihn als Prozess startet (JSON auf stdin, Umgebung aus einer Fixture, Exit, stdout und stderr getrennt), Varianten als Tabelle gegen ein Orakel laufen und wenige Durchstiche die Verdrahtung über `core.hooksPath` belegen. Grundlage: Messung und Empfehlung vom 12.09.2026 (`empfehlung-testumgebung-fable.md`), 65 von 113 bats-Dateien rufen ohnehin Python, Isolation ist in pytest eine Fixture statt 113 handgeschriebener `setup()`-Blöcke.
