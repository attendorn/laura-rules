...
- **Aggregat-Klassifikations-Disziplin (Sample-Size-Disziplin).** Bei jedem Klassifikations-Tag (VERIFIZIERT/NUTZBAR) auf einem AGGREGAT (Sprint, Multi-Agent, „alle Module", Bündel, Sammel-Commit) MUSS die Stichprobengröße im ±10-Zeilen-Kontext stehen: `n=X`, `X von Y`, `X/Y`, `X% Echt-Loop`. Hook `check-sample-vs-tag.sh` blockt Edits in Tageslog/Memory ohne Mengenangabe. Wurzel-Bias: `sample_size_blind`. [H3]
...
- **Read-Token-Limit-Pflicht.** Wenn ein `Read`-Tool-Call mit `"exceeds maximum allowed tokens"` fehlschlägt, MUSS die Datei in Chunks über `offset`/`limit` nachgeladen werden — Default 150 Zeilen pro Chunk. Verboten: Datei überspringen, Zusammenfassung aus anderer Quelle als Ersatz nehmen, „reicht erstmal"-Annahme. Die Information die gerade gebraucht wird kann im ungeladenen Teil stehen. Hook `~/Laura/code/hooks/check-read-token-limit.sh` (PostToolUse/Read) injiziert die Pflicht-Anweisung in den nächsten LLM-Turn. [H5]
...
- **Sekundärquellen prüfen, bevor aus ihnen geschlossen wird (blind_trust_internal_sources).** Gilt für **jede Quelle, die den Bestand nur abbildet statt ihn zu sein**: Index-/Übersichts-Dateien (DATEI_INDEX.md, KUNDENPROFIL.md, projekte-aktiv.md), **DB-Spiegel** (`provinzial_mail`), **eingefrorene oder abgelöste Tabellen** (`aufgabe`), **generierte Caches** (MEMORY.md) und **Dateisystem-Suchen über Symlinks** (`find` ohne `-L` unter `~/Provinzial` liefert still 0 Treffer statt eines Fehlers). Diese werden NICHT ungeprüft als aktuelle Wahrheit übernommen. Vor Übernahme in Kunden-Kommunikation oder Konzepte: (a) Header-Datum / mtime prüfen – wenn älter als 60 Tage, gegen Primärquelle validieren, (b) explizite Frontmatter-Info „Stand:" suchen, (c) bei Zweifel Florian fragen. **Der eigentliche Kern: Vor jeder Aussage über einen Bestand („es gibt keinen X", „X ist überfällig", „X wurde nie beantwortet") zuerst prüfen, ob die benutzte Quelle diesen Ausschnitt überhaupt abbilden KANN.** Ein leeres Ergebnis ist kein Beweis für Abwesenheit, solange die Reichweite der Quelle ungeprüft ist. Bekannte Teilsichten stehen in `memory/topics/teilsichten.md` und werden beim Lesen der jeweiligen Quelle mitgelesen; neu entdeckte gehören dort eingetragen. [H11]
...
- **Inventur-Quelle = Definitive Liste, nicht Verzeichnis-Glob.** Bei jeder „komplett"-Aussage über eine Menge (Hooks, Skills, Cron-Jobs, MCP-Server, Permissions, Dateien) MUSS die Quelle die kanonische Registrierung sein, nicht ein Glob über erwartete Verzeichnisse. Konkret: Hooks → `~/.claude/settings.json` parsen. Skills → SKILL.md-Frontmatter aus allen geladenen Plugin-Sourcen. Cron-Jobs → `crontab -l` + launchd-Plists. MCP-Server → `~/.claude.json` mcpServers-Block. Glob über Dateien deckt nur die Files im erwarteten Pfad ab, nicht die tatsächlich aktiven. Wurzel-Bias: `sample_size_blind`. [H14]
...
- **Ursache vor Hook** (Bias-Muster: `overcompletion_framing` u.a.)**.** Wenn ein inhaltliches Bias-Pattern (overcompletion_framing, sample_size_blind, blind_trust_internal_sources, initiative_deficit, meta_reactive_learning, etc.) 3+ Reviews in Folge auftritt, ist eine Ursachen-Analyse PFLICHT bevor ein Hook gebaut wird. Drei-Schichten-Diagnose:
...
- **Mess-Disziplin bei Diagnosen.** Gilt für jede Aussage, die aus einer eigenen Messung, einem Test oder einem Experiment abgeleitet wird — nicht für Klassifikations-Tags (die decken die Sample-Size-Regeln oben ab), sondern für empirische Befunde selbst:
...
- **Befund erledigen und melden, nicht melden statt erledigen.**
  ...
  **Abgrenzung zu `voice.md`** („erledigt Selbstverständliches still"): Das gilt für Beiwerk im
  ...
  Reparatur an Code, Werkzeug, Hook oder Vorlage ist kein Beiwerk und wird genannt. [H25]
...
Kritische Regeln sind zusätzlich per Hooks/Deny-Liste erzwungen (settings.json). Die Git-Wächter (Commit-Pfadliste, Push-Allowlist, pre-push-Tests) stehen bei den Coding-Regeln in `rules/coding.md`:
